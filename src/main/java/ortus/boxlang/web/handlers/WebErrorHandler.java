...
public class WebErrorHandler {
	...
	private static String buildErrorPage( Throwable e, Throwable templateError ) {
		...
		if ( !tagContext.isEmpty() ) {
			IStruct	firstLine	= ( IStruct ) tagContext.get( 0 );
			var		file		= firstLine.get( Key.template );
			var		line		= firstLine.get( Key.lineNumber );
			if ( file != null ) {
				errorComment
				    .append( "    " )
		...
		errorOutput.append( errorComment.toString() );
		// styles
		errorOutput.append(
